...
async def process_add_movie(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(text="Send the title of the movie you want to add 🎬", reply_markup=builder.as_markup())
...
async def process_movie_title(message: types.Message, state: FSMContext):
    ...
    if result:
        ...
        builder = back_button("add_movie")

        await message.answer("Movie not found. Try a more precise name", reply_markup=builder.as_markup())
